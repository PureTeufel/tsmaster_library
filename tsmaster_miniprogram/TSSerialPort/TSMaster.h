...
typedef s32 (__stdcall* Tpanel_create_control)(const char* APanelName, const char* AParentCtrlName, const s32 AParentCtrlSubIdx, const char* AName, const TLIBPanelControlType ACtrlType, const float ALeft, const float ATop, const float AWidth, const float AHeight, const TLIBPanelSignalType AVarType, const char* AVarLink);
...
typedef struct _TTSApp {
    ...
    native_int FDummy[626]; // >>> mp app end <<<
...
typedef struct _TTSCOM {
    ...
    native_int FDummy[659]; // >>> mp com end <<<
    // internal functions
